...
import interfaceservice.LocationFinder;
...
public class GoogleMapsClient implements LocationFinder {
     ...
     * @throws InvalidPostalCodeException if no nearby locations are found
     * @throws Exception if the LLM json response fails
     */
    ...
    private JSONObject fetchDetails(String placeId) throws Exception {
        final String url = String.format(
                "https://maps.googleapis.com/maps/api/place/details/json?place_id=%s&fields=name,geometry,"
                        + "formatted_address,reviews&key=%s",
                placeId, apiKey
        );
